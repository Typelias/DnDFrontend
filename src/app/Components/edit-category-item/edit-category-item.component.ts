...
export class EditCategoryItemComponent implements OnInit {
  ...
  constructor(private ac: ActiveCampaignService, private dialogRef: MatDialogRef<EditCategoryItemComponent>,
              @Inject(MAT_DIALOG_DATA) private helper: IndexHelper) {
    this.newItem.name =
      this.ac.activeCharacter.otherProficienciesAndLanguages.categories[this.helper.catIndex].items[this.helper.itemIndex].name;
    this.newItem.description =
      this.ac.activeCharacter.otherProficienciesAndLanguages.categories[this.helper.catIndex].items[this.helper.itemIndex].description;
    this.newItem.amount =
      this.ac.activeCharacter.otherProficienciesAndLanguages.categories[this.helper.catIndex].items[this.helper.itemIndex].amount;
    this.newItem.showAmount =
      this.ac.activeCharacter.otherProficienciesAndLanguages.categories[this.helper.catIndex].items[this.helper.itemIndex].showAmount;
  }
